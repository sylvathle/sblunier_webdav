...
int main(int argc, char **argv )
{
	...
	Float_t vals[] = {atoi(argv[1]),atoi(argv[2])};
	Int_t colors[] = {3,2};
	Int_t nvals = sizeof(vals)/sizeof(vals[0]);

	TCanvas *cpie = new TCanvas("cpie","TPie test",700,300);

	TPie *pie3 = new TPie("pie1","davix results",nvals,vals,colors);

	//pie3->SetX(.32);
	pie3->SetCircle(.35,.5,.3);
	pie3->SetAngle3D(90.);
	pie3->SetEntryLabel(0,"Davix Ok");
	pie3->SetEntryLabel(1,"Davix Failed");
	pie3->SetLabelsOffset(-.1);
	pie3->SetLabelFormat("%perc");
	pie3->Draw("3d t nol");
	TLegend *pieleg = pie3->MakeLegend();
	pieleg->SetY1(.20); pieleg->SetY2(.80);
